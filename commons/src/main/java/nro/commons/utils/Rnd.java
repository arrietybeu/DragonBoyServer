...
public class Rnd {
    ...
    /**
     * @see RandomGenerator#nextGaussian()
     */
    public static boolean chancePercent(double percent) {
        return percent >= 100 || (percent > 0 && chance() < percent);
    }

    /**
     * Trả về true với xác suất part/total * 100%.
     *
     * @param part  phần tử (ví dụ 10)
     * @param total tổng phần (ví dụ 100)
     * @return true nếu random nằm trong xác suất, false nếu không
     */
    public static boolean chancePercent(double part, double total) {
        if (part <= 0 || total <= 0) return false;
        if (part >= total) return true;
        return chance() < (part / total) * 100;
    }
