...
public class Cui extends Npc {
    ...
    public void openUIConfirm(Player player, int select) {
        ...
        if (player.getPlayerContext().isBaseMenu()) {
            switch (select) {
                case 0 -> areaService.changerMapByShip(player, ConstMap.TRAM_TAU_VU_TRU_TRAI_DAT, Rnd.nextInt(400, 444), 5,  1);
                case 1 -> areaService.changerMapByShip(player, ConstMap.TRAM_TAU_VU_TRU_NAMEC, Rnd.nextInt(400, 444), 5, 1);
            }
        }
    }
