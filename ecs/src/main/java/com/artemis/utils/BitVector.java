...
public class BitVector {
    ...
    public IntBag toIntBag(IntBag out) {
        ...
        for (int i = 0, index = 0; count > index && i < words.length; i++) {
            long bitset = words[i];
            int wordBits = i << 6;
            while (bitset != 0) {
                ...
                if (index >= data.length) {
                    out.ensureCapacity(index + 1); // fix chính
                    data = out.getData(); // reload lại vì có thể re-alloc
                }
        ...
        out.setSize(count); // đảm bảo size chuẩn
        return out;
    }
    ...
    private int prepareBag(IntBag out, int elementsPerEntry) {
        ...
        out.ensureCapacity(count + 8); // mở rộng thêm buffer tránh sát biên
        out.setSize(count);
        return count;
    }
