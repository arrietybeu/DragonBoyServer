...
public class AccountDAO {
    ...
    public static Account getAccount(String username, String password) {
        ...
        if (accountRef.get() != null) {
            AccountTime accountTime = getAccountTime(accountRef.get().getId());
    ...
    public static AccountTime getAccountTime(int accountId) {
        ...
        Database.select("SELECT * FROM account_time WHERE account_id = ?", rs -> {
            if (rs.next()) {
                ...
                accountTime.setBanUntil(rs.getTimestamp("accumulated_rest"));
            }
