...
public class BannedIpDAO {
    ...
    public static boolean insert(BannedIP bannedIP) {
        return Database.insertUpdate("INSERT INTO banned_ip(mask, time_end) VALUES (?, ?)", ps -> {
    ...
    public static boolean update(BannedIP bannedIP) {
        return Database.insertUpdate("UPDATE banned_ip SET mask = ?, time_end = ? WHERE id = ?", ps -> {
            ps.setString(1, bannedIP.getMask());
            ps.setTimestamp(2, bannedIP.getTimeEnd() == null ? null : bannedIP.getTimeEnd());
    ...
    public static boolean remove(String mask) {
        return Database.insertUpdate("DELETE FROM banned_ip WHERE mask = ?", ps -> {
            ps.setString(1, mask);
            ps.execute();
        });
    }
    ...
    public static Set<BannedIP> getAllBans() {
        ...
        Database.select("SELECT * FROM banned_ip", rs -> {
            while (rs.next()) {
    ...
    public static void cleanExpiredBans() {
        Database.insertUpdate("DELETE FROM banned_ip WHERE time_end < current_timestamp AND time_end IS NOT NULL");
    }
