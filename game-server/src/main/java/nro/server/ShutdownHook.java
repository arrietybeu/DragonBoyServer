...
public class ShutdownHook extends Thread {
    ...
    private ShutdownHook() {
        if (ShutdownConfig.RESTART_SCHEDULE != null) {
            // CurrentThreadRunnableRunner, otherwise
            // ThreadPoolManager.getInstance().shutdown() will try to wait for this cron
            // task
            CronService.getInstance().schedule(() -> System.exit(ExitCode.RESTART), CurrentThreadRunnableRunner.class,
                    ShutdownConfig.RESTART_SCHEDULE, true);
            log.info("Scheduled automatic server restart based on cron expression: {}",
                    ShutdownConfig.RESTART_SCHEDULE);
        }
    }

    @Override
    public void run() {
        // this method is run when System.exit is triggered, or via other external
        // events like console CTRL+C
        ...
        GameServer.shutdownNioServer(); // shuts down nro.utils.test.network, disconnects cs/ls/all players and saves
                                        // them
    ...
    protected void initShutdown(int exitCode, int delaySeconds) {
        ...
        if (previousValue == UNSET_DELAY)
            Thread.startVirtualThread(() -> System.exit(exitCode)); // async since System.exit indefinitely blocks the
                                                                    // calling thread
    }

    /**
     * @param remainingSeconds - remaining time in seconds, until the shutdown will
     *                         be performed
     * @param minInterval      - minimum interval to be returned (minInterval will
     *                         equal remainingSeconds if remainingSeconds is
     *                         shorter)
     * @param maxInterval      - maximum interval to be returned
     * @return The interval (in seconds) to wait until the next announce should be
     *         sent to all players.
     */
