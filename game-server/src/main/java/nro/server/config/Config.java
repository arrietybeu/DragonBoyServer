...
public class Config {
    ...
    private static final List<Class<?>> CONFIGS = Arrays.asList(NetworkConfig.class, PacketFloodFilterConfig.class);
...
//        properties.putAll(EventService.getInstance().getActiveEventConfigProperties());
        ...
        if (NetworkConfig.CLIENT_CONNECT_ADDRESS.getAddress().isAnyLocalAddress()) {
            ...
            if (localIPv4 == null)
                throw new RuntimeException("No IP for Aion client advertisement configured and local IP discovery failed. Please configure gameserver.network.client.connect_address");
